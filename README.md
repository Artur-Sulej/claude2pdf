...
- **Modern PDF Output**: Generates clean, readable PDFs via `wkhtmltopdf`.
- **Smart Defaults**: Automatically names output files based on input filenames.

## Prerequisites

This tool requires `wkhtmltopdf` to be installed on your system.
...
5. **PDF Generation**: Uses `wkhtmltopdf` to render the final PDF document.
